...
.class('Application').uses(WithEvents).define({
    ...
    __fluent__route: function(r)
    {
        ...
        // We have a route, throw up a controller for it
        var TController = route.controller;
        if (typedef.is(this.currentController, TController)) {
            // controller already instantiated...
        } else {
            this.currentController = new TController();
        }

        this.currentController.handleRoute();
