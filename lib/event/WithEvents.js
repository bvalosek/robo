...
.mixin('WithEvents') .define({
    ...
    // to react to it
    on: function(name, callback, _context)
    {
        if (!this.__events)
    ...
    __fluent__listenTo: function(obj, name, callback)
    {
        ...
        if (!obj.__listenerId)
            Object.defineProperty(obj, '__listenerId', {
                configurable: true, enumberable: false,
                writable: true, value: _.uniqueId('l')
            });

        var id = obj.__listenerId;
        listeningTo[id] = obj;
    ...
    __fluent__stopListening: function(_obj, _name, _callback)
    {
        ...
        if (_obj)
            (listeningTo = {})[_obj.__listenerId] = _obj;
