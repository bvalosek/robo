...
.class('ObservableObject') .uses(WithObservableProperties) .define({
    ...
    getComputedValue: function(prop, compFn, _args)
    {
        ...
        // Push our stuff onto the stack to track all dependencies. If we're
        // calling this (for setters, then do it)
        ...
        this.__deps[prop] = frame; // track all of these bad boys
        return val;
    },
    ...
    __ondefine__: function(C)
    {
        _(C.__signature__).each(function(info, key) {
            if (info.decorations.OBSERVABLE || info.decorations.COMPUTED) {
                ...
                // if it is a getter/setter
                } else if (_(value).isObject() && (value.get || value.set)) {
                    ObservableObject.addProperty(target, key, value);

                    // get does its thing, but gotta check on sets to see if
                    // the value needs to trigger a change or WUT
                    Object.defineProperty(target, _key, {
                        configurable: true, enumberable: false,
                        get: function() {
                ...
                // just some normal property value
                } else {
                    ObservableObject.addProperty(target, key, value);
