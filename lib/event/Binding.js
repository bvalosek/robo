...
.class('Binding') .uses(WithEvents) .define({
    ...
    // one way or two way binding (two way by default)
    mode: 0,

    __static__readonly__TWO_WAY: 0,
    __static__readonly__ONE_WAY: 1,
    ...
    __fluent__setTarget: function(target, prop)
    {
        ...
        // Assign the target our current value, and bind all further changes to
        // change the target
        ...
        // two way and we can listen to target -> listen for changes to force
        // source value back. Hopefully this doesnt not result in an infinite
        // loop...
        if (this.mode == Binding.TWO_WAY &&
            typedef.is(target, WithObservableProperties)) {
    ...
    // Ensure that we can prevent the target from propigating back to the
    // source via setting 1-way
    __fluent__setMode: function(mode)
    {
        if (mode === this.mode || mode > 1 || mode < 0)
            return this;

        if (this._targetSet)
            throw new Error('Cannot change mode after target has been bound');
    ...
    __fluent__clearTargets: function()
    {
        ...
        // TODO this doesn't clear out the listenings to targets for 2 way
        // bindings, THESE WILL PERSIS
