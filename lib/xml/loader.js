...
var IDictionary = require('../model/IDictionary');

var typedef    = require('typedef');

var XHTML_NS = 'http://www.w3.org/1999/xhtml';
var ROBO_NS  = 'http://www.robojs.org/v1/schema';
...
function(node, _opts, lastViewGroup)
{
    ...
    _(node.nodes).each(function(child) {
        ...
        var oKey = child.attrib ? child.attrib[ROBO_NS + '/key'] : null;
        ...
        // thus the content prop. If this is an attribute child, then
            ...
            handleProp(obj, undefined, o, oKey);
...
function handleProp(obj, key, value, vKey)
{
    // have to infer the key to use
    if (key === undefined) {

        // If this is a list, add the value
        if (typedef.is(obj, IList))
            obj.add(value);
        else if (typedef.is(obj, IDictionary))
            obj.add(vKey, value);

        // If we have a defined content property, rock that.
        var contentProp;
        ...
        if (contentProp) {
            handleProp(obj, contentProp, value, vKey);
        }

    } else {
        if (typedef.is(obj[key], IList))
            obj[key].add(value);
        else if (typedef.is(obj, IDictionary))
            obj[key].add(vKey, value);
        else
            obj[key] = value;
